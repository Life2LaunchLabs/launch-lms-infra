...
After cutover, dispatch `Roll back operations apex DNS` on protected `main` to restore the existing A record to the legacy unstable host. The host apply workflow detects the rolled-back target and refuses a later apply unless `acknowledge_operations_rollback=true` is explicitly selected. Review a topology update and a fresh plan before acknowledging reapplication.

During the DNS cache window, certificate validators may still reach the former host. The protected `Bridge operations ACME during DNS propagation` workflow can temporarily proxy only the apex HTTP challenge path to the new host while preserving the legacy redirect for every other path. Dispatch `action=enable`, verify public TLS, then dispatch `action=disable` to remove the bridge. The workflow changes no DNS record and prints no legacy gate credentials.
